...
import edu.stanford.rsl.conrad.numerics.DecompositionSVD;
	...
	public void doImageUndstortion(){
		...
		NumericPointwiseOperators.subxDist.setAtIndex(x, y,(float)(xDist.getAtIndex(x,y)+u_vec.get.Element(cc)*Math.pow(x, k)*Math.pow(y, l)));tractBy(shiftedR, (float) (maxR * 0.5));
		...
		for(int i = 0; i < ny; i++)
		{
			for(int j = 0; j < nx; j++)
			{
				...
				Xu2.setElementValue(j,i,X.getAtIndex((int)((i+l)*fy),(int)((j+l)*fx)));
				Yu2.setElementValue(j,i,X.getAtIndex((int)((i+l)*fy),(int)((j+l)*fx)));
				Xd2.setElementValue(j,i,X.getAtIndex((int)((i+l)*fy),(int)((j+l)*fx)));
				Yd2.setElementValue(j,i,X.getAtIndex((int)((i+l)*fy),(int)((j+l)*fx)));
		...
		for(int x = 0; x < imSize; x++)
		{
			for(int y = 0; y < imSize; y++)
			{
				...
				for(int k = 0; k <= degree; k++)
				{
					...
					{
						// TODO
						xDist.setAtIndex(x, y,(float)(xDist.getAtIndex(x,y)+u_vec.get.Element(cc)*Math.pow(x, k)*Math.pow(y, l)));
						yDist.setAtIndex(x, y,(float)(xDist.getAtIndex(x,y)+v_vec.get.Element(cc)*Math.pow(x, k)*Math.pow(y, l)));
						CC++;
					}
				}
			}
		...
		for(int i = 0; i < imSize; i++)
		{
			...
			{
				float val=InterpolationOperators.interpolateLinear(distorted, xDist.getAtIndex(i,j),yDist.getAtIndex(i,j));
